...
var _ = require('lodash'), parseSearch, isUniqueIssue, isSendObject, matchEmbed, CSVIEW = '$b.csview';
...
module.exports = {
	generate : function (name,pathName,ext,opts,sendObjectInit) {
		...
		allowedEmbed = opts.embed === true ? opts.embed : _.indexBy(opts.embed||[]);
		...
		baseCtrlr = {
		    ...
		    if ( req.body === undefined ) {
		        require('express').json()( req, res, me );
		    } else {
		        me();
		    }
			},
			...
			getProperty: function (req,res,next) {
				...
				filter = req.param(CSVIEW) || "public", property = req.param("prop123var");
			...
			destroy: function(req,res,next) {
				req.booster.models[name].destroy(req.params[pathName],req.booster.parent,req.param("force") === "true",function(err,data){
					if (err) {
						res.send(err.delete === "children" ? 409 : 400, err);
					} else if (!data) {
						res.send(404);
					} else {
						res.send(204);
