...
module.exports = {
	generate : function (name,pathName,ext,opts) {
		var ctrlr = {}, parent, parentProperty, parentDefault, checkParentProperty, baseCtrlr;
		opts = opts || {};
		...
		baseCtrlr = {
			...
			update: function(req,res,next) {
				var me = function () {
					if (!checkParentProperty(req,"update")) {
						...
						req.booster.models[name].update(req.params[pathName],req.body,req.booster.parent,function(err,data){
							if (err) {
								// check for conflict vs other error
								res.send(isUniqueIssue(err)?409:400,err);
							} else if (!data) {
			...
			patch: function(req,res,next) {
				var me = function () {
					if (!checkParentProperty(req,"patch")) {
						...
						req.booster.models[name].patch(req.params[pathName],req.body,req.booster.parent,function(err,data){
							if (err) {
								// check for conflict vs other error
								res.send(isUniqueIssue(err)?409:400,err);
							} else if (!data) {
			...
			destroy: function(req,res,next) {
				req.booster.models[name].destroy(req.params[pathName],req.booster.parent,function(err,data){
					if (err) {
						next(err);
					} else if (!data) {
						res.send(404);
		...
		if (opts.only) {
			ctrlr = _.pick(ctrlr,[].concat(opts.only));
		} else if (opts.except) {
