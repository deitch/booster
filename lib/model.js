	...
	generate : function(name,models,db,module) {
		...
		patchFarSide = function (m,name,id,type,model,field,cb) {
			...
			async.waterfall([
				...
				function (data,cb) {
					...
					data = [].concat(data||[]);
					if (data.length > 0) {
						models[m].patch(_.pluck(data,rid),update,cb);
					} else {
						cb(null,null);
					}
				},
		...
		patchMM = function (m,name,id,type,model,field,thru,cb) {
			...
			async.waterfall([
				...
				function (data,cb) {
					// remove all of them
					async.each(_.pluck(data,rid),function (i,cb) {
		...
		doEmbed = function (set,embed,callback) {
			...
			async.each(set,function (item,cb) {
								...
								async.waterfall([
									...
									function (res,cb) {
										...
										if (res && res.length > 0) {
											...
											ids = _.pluck(res,getFarSide(thru,m,"belongs_to"));
					...
					if (err) {
						...
						db.get(table,key,function (err,res) {
							...
							if (err) {
								...
								// get only those that match the parent - but remove the embed
								search = util.removeDollarB(parent);
								set = search && _.keys(search).length > 0 ? sjs.matchArray(res?[].concat(res):[],search) : [].concat(res);
								// pass it through validate
								async.concat(set,function (r,cb) {
									validate(key,r,"get",cb);
								},function (errors,verrs) {
									var errs = verrs && verrs.length > 0 ? verrs : null, data = set.length > 1 ? set : set[0], embed = util.extractEmbed(parent);
									// do we do any embedding?
									if (!errs && embed) {
										doEmbed(set,embed,function () {
						...
						db.find(table,fulls,function (err,res) {
							if (err) {
								...
								async.concat(res||[],function (r,cb) {
									...
									var errs = verrs && verrs.length > 0 ? verrs : null, embed = util.extractEmbed(parent);
									// do we do any embedding?
									if (!errs && embed) {
										doEmbed(res,embed,function () {
				...
				validate(null,model,"create",function(e,verrs){
					if (verrs) {
						...
						checkUnique(model,null,false,function (errors) {
							if (errors) {
								callback(suppressUniqueError?null:errors,model);
							} else {
									...
									if (err) {
										...
										doPresave(model,function (errors,res) {
											if (errors) {
												...
												db.create(table,util.removeAssociations(util.removeDollarB(model),fields),function (err,res) {
													setRemoteAssociations(model,res,"create",function () {
								...
								if (errors) {
									...
									filters.update(key,model,models,function (err) {
										if (err) {
											...
											db.update(table,key,util.removeAssociations(util.removeDollarB(model),fields),function (err,data) {
												...
												if (err) {
													...
													setRemoteAssociations(model,key,"update",function () {
									...
									if (errors) {
										...
										filters.update(key,model,models,function (err) {
											if (err) {
												...
												db.patch(table,key,util.removeAssociations(util.removeDollarB(model),fields),function (err,data) {
													// now check if there is any cascading to be done
													if (err) {
														...
														setRemoteAssociations(model,key,"patch",function () {
															checkCascade(key,model,function () {
																pps.patch(key,model,models,err,data,function (e) {
																	callback(e||err,data);
																});
