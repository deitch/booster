	...
	generate : function(name,models,db,module) {
				...
				_.each(fields,function(value,key){
					...
					if (value !== undefined) {
						...
						if (attrs[key] === undefined) {
							...
							if (value.required && (mode === "get" || mode === "find" || (mode === "create" && !value.createoptional))) {
								err[key] = "required";
							}
						} else if (value.mutable === false && mode === "update") {
							err[key] = "immutable";
		...
		M = {
			...
			create: function (model,callback) {
				// validate
				var errors = validate(model,"create");
				if (errors) {
			...
			update: function (key,model,callback) {
				// validate
				var errors;
				model = model || {};
