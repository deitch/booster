	...
	generate : function(name,models,db,module) {
		...
		M = {
			...
			create: function (model,parent,callback) {
				...
				// need a clean model without $b fields
				cleanmodel = getCleanmodel(model);
				// validate
				validate(null,cleanmodel,"create",function(e,verrs){
					if (verrs) {
									...
									if (err) {
										...
										doPresave(model,function (errors,res) {
											if (errors) {
												...
												db.create(table,cleanmodel,function (err,res) {
			...
			update: function (key,model,parent,callback) {
				var cleanmodel, mutreq = [], nextStep = function() {
					// check for uniques
								...
								if (errors) {
									...
									filters.update(key,model,models,function (err) {
										if (err) {
											...
											db.update(table,key,cleanmodel,function (err,data) {
				...
				cleanmodel = getCleanmodel(model);
				// validate
				validate(key,cleanmodel,"update",function (e,errors) {
						...
						if (mutreq.length > 0) {
							db.get(table,key,function (err,res) {
								if (res) {
									...
									cleanmodel = _.extend({},_.pick(res,mutreq),cleanmodel);
									nextStep();
								} else {
									callback(err,model);
								}
			...
			patch: function (key,model,parent,callback) {
				...
				cleanmodel = getCleanmodel(model);
				key = key === 'undefined' || key === null ? model[id] : key;
				validate(key,cleanmodel,"patch",function (e,errors) {
					if (errors) {
						callback(errors,model);
					} else {
						// check for uniques
									...
									if (errors) {
										...
										filters.update(key,model,models,function (err) {
											if (err) {
												...
												db.patch(table,key,cleanmodel,function (err,data) {
													// now check if there is any cascading to be done
													if (err) {
