...
loadController = function(name,pathName,path,models,opts,config) {
	var ret, module;
	/*jslint stupid:true */
	...
	ret = controllerFactory.generate(name,pathName,module,opts);
	return ret;
};
	...
	resource: function(resourceName) {
		...
		if (name) {
			...
			_.each(optsAry,function (opts,i) {
				var basePath = "", adder = "", pathName = opts.name || name,
				c = loadController(name,pathName,config.controllers,models,opts,config.param),
				...
				_.each(ctrlrRoutes.base,function (route,method) {
					var verb = route[0].toLowerCase(), path = basePath+(route[1] ? adder+':'+pathName : ''), filter = (c.filter||{})[method],
				...
				_.each(c.properties||{},function (handles,prop) {
					if (handles) {
						if (handles.get && typeof(handles.get) === "function") {
							app.get(basePath+adder+':'+pathName+'/'+prop,middleware,handles.get);
						}
						if (handles.set && typeof(handles.set) === "function") {
							app.put(basePath+adder+':'+pathName+'/'+prop,middleware,handles.set);
						}
					}
				});
				...
				_.each(opts.resource,function (types,prop) {
					var fullPath = basePath+adder+':'+pathName+'/'+prop;
				...
				_.each(ctrlrRoutes.property,function (route,method) {
					...
					if (c[method] && typeof(c[method] === "function")) {
						app[verb](basePath+adder+":"+pathName+'/:prop123var',middleware,c[method]);
					}
				});
			});
		}
