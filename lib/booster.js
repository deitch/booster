		...
		if (name) {
			...
			_.each(optsAry,function (opts,i) {
				...
				_.each(ctrlrRoutes.base,function (route,method) {
					var verb = route[0].toLowerCase(), path = basePath+(route[1] ? adder+':'+pathName : '')+format, filter = (c.filter||{})[method],
				...
				_.each(opts.resource,function (types,prop) {
					var fullPath = basePath+adder+':'+pathName+'/'+prop+format;
