...
loadModel, loadController, loadGlobalFilters, trailSlash, trim, models = {}, controllers = {}, globalFilters = {}, routes = {}, ctrlrRoutes = {
	base: {
		index: ["GET",false],
		show: ["GET",true],
		create: ["POST",false],
	...
	resource: function(resourceName) {
		...
		loadPath = function (parentPath,opts) {
			...
			_.each(opts.resource,function (types,prop) {
				var fullPath = basePath+adder+':'+pathName+'/'+prop+format, method, gf, mw, post, pp, 
				...
				if (_.contains(types,"get")) {
					...
					// we will add post-processors later; once again, we need to look at the controller
					//post = (c.post||{})[method];
					//pp = [].concat(processors,post || []);
