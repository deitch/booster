...
* `prevent`: if any children are found, the `DELETE` will return a `403`. The only way to delete this resource is to delete all of its children first.
* `forcece`: if any children are found, the `DELETE` will return a `403`, just like `prevent`. However, if the query parameter `force` is set to `true`, i.e. `?force=true`, it will delete the resource and leave the children alone.
